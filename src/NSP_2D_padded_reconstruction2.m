function [reconstructed_data] = NSP_2D_padded_reconstruction2 (padded_c_0, padded_details)
  ...
  length_data=length(padded_details{J})/5; 
  ...
  for l=1:J
    ...
    [rod,cod]=size(refined_data_odd);

    %merging the even and the odd refinements
    refined_data_x=[refined_data_ev(:,1)';refined_data_odd(:,1)'];
    refined_data_x=refined_data_x(:);
    refined_data_y=[refined_data_ev(:,2)';refined_data_odd(:,2)'];
    ...
    [r1,c1]=size(refined_data); %%
    [r2,c2]=size(padded_details{l}); %%%%
  ...
  length_data=length(padded_reconstructed_data)/5;
  reconstructed_data=padded_reconstructed_data(2*length_data+1:end-(2*length_data),:);