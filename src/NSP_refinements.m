function [f_refinemented] = NSP_refinements (data, K, mask_ev, mask_odd)
...
%{
  ...
  %f=@(x) sin((7*x))%+exp(-7*x)%+x.*cos(7*x);%*pi/(N*2^-(J+1)));
  ...
  plot(x_data_for_data,data,'.','MarkerSize',15,'Color','k','LineStyle','none')%,'-*')
  hold on
