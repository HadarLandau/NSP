function [c_0, details,refined_c_l, padded_c_0, padded_details] = NSP_2D_padded_decomposition (data, J, all_alpha_ev, all_alpha_odd, all_shifts)
  ...
  %%%% computing C_l-1 by gamma[l-1] %%%%
    ...
    lg=length(gamma)
    for i=1:length((downsampled_c_l))
      for j=-ceil(lg/2)+1:floor(lg/2)
        if i+j>0 && i+j<=length(gamma)
          prev_c_l(i,:)= prev_c_l(i,:)+downsampled_c_l(i+j,:).*gamma(j+ceil(lg/2));
        end
      end
    end
    %}
  ...
  hold on
  %S_alpha[l-1](C_l-1)
  plot(refined_c_l_x((N-1)*2^J:end-N*2^J),refined_c_l_y((N-1)*2^J:end-N*2^J),...
