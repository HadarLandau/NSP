function [c_0, details, refined_c_l] = NSP_2D_not_padded_decomposition2 (data, J)
  %only 1 iteration of alpha each step
...
%  padded_data=[data(3:end,:);data;data;data;data(1:end-2,:)];
  ...
  for l=J:-1:1
    ...
    if l==J
      [alpha_ev, alpha_odd] = NSP_create_mask_a1_2 (cos(2*pi/(L/2)));
    elseif l==J-1
      'ffa'
      [alpha_ev, alpha_odd] = NSP_create_mask_a1_2 (cos(2*pi/((L-0.5)/2)));
    elseif l==J-2
      'aaa'
      [alpha_ev, alpha_odd] = NSP_create_mask_a1_2 (cos(2*pi/((L-0.75)/2)));
    elseif l==J-3
      'fff'
      [alpha_ev, alpha_odd] = NSP_create_mask_a1_2 (cos(2*pi/((L-0.875)/2)));
    else
      'ggg'
      [alpha_ev, alpha_odd] = NSP_create_mask_a1_2 (cos(2*pi/((L-0.9375)/2)));
    end
 ...
 %         prev_c_l(i,:)
     ...
     prev_c_l(i,1).^2+prev_c_l(i,2).^2-1
    end
    %}
